...
export async function seedDatabase(): Promise<void> {
  ...
  try {
    ...
    // W1 Task / X2: Find or create users by natural key (email) to ensure idempotency
      const userData = [
    ...
    // W1 Task / X2: Find or create projects by natural key (name) to ensure idempotency
      const projectData = [
      ...
      const projects = await projectData.reduce(async (accPromise, item) => {
        ...
        const project =
          existing ?? (await projectRepository.save(projectRepository.create(item)));
        acc.push(project);
        return acc;
     ...
     // W1 Task / X2: Find or create project memberships by composite key (userId, projectId) to ensure idempotency
      const memberData = [
        {
          userId: ali.id,
      ...
      await memberData.reduce(async (prevPromise, item) => {
        ...
        if (!existing) {
          await projectMemberRepository.save(projectMemberRepository.create(item));
        }
      }, Promise.resolve());

    // W1 Task / X2: Find or create tags by natural key (name) to ensure idempotency
      const tagData = [
        { name: "backend" },
        { name: "database" },
        { name: "urgent" },
     ...
     // W1 Task / X2: Find or create tasks and relations by natural key (title, projectId) to ensure idempotency
      const taskData = [
        {
      ...
      // W1 Task / X2: Find or create comments by natural key (taskId, authorId, body) to ensure idempotency
      const commentData = [
        {
          taskId: tasks[0].id,
          authorId: ali.id,
